...
Autor: Edwin H. Calderón García

Módulo portado desde `Tarea 4/Pregunta 3/app_p3.py` al armazón del tablero.
Los cambios respecto al original son únicamente de integración: la ruta del
archivo de datos se resuelve contra la ubicación de este archivo, el layout
pasa a ser una variable del módulo en lugar de asignarse a una app propia, y
los callbacks se registran con `@callback` en vez de `@app.callback`. La lógica
de análisis y el diseño de las visualizaciones no se modificaron.

El logo que usa el layout vive en `despliegue/assets/logo_invias.png`, la
carpeta de assets del armazón.
"""
...
df = pd.read_csv(
    RAIZ / "Tarea 2" / "Pregunta 3" / "df_analitico_p3.csv"
)
